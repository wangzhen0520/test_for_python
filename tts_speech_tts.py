...
def submit_tts(create_time, voice_id, device_secret, text_name, text):
    nonce = str(uuid.uuid4()).replace("-", "")
    timestamp = int(round(time.time() * 1000))
    ...
    # print(response.text)
    path = f"voiceid/{create_time}/{voice_id}"
...
async def test_submit():
    ...
    # voice_id_list = ["xijunma"]
    # text_list = [['网络不稳定,请稍后重试.mp3', '网络不稳定,请稍后重试'], ['授权失败了,请重新授权.mp3', '授权失败了,请重新授权'], ['网络离线了,我听不懂你说什么.mp3', '网络离线了,我听不懂你说什么'],
    #              ['鉴权失败了,请重新授权.mp3', '鉴权失败了,请重新授权'], ['响应超时了,暂时不能为您服务.mp3', '响应超时了,暂时不能为您服务'], ['抱歉帮不了您, 有需要再叫我哦.mp3', '抱歉帮不了您, 有需要再叫我哦'],
    #              ['抱歉没听懂你说什么, 有需要再叫我哦.mp3', '抱歉没听懂你说什么, 有需要再叫我哦'], ['语音资源初始化失败了, 请稍后重试.mp3', '语音资源初始化失败了, 请稍后重试'],
    #              ['欢迎使用方太语音助手，请用你好方太唤醒我.mp3', '欢迎使用方太语音助手，请用你好方太唤醒我'], [
    #                  '饮品已制作完成.mp3', '饮品已制作完成'], ['消毒完成啦.mp3', '消毒完成啦'], ['清洁完成啦.mp3', '清洁完成啦'],
    #              ['出水完成啦.mp3', '出水完成啦'], ['接水盒已满，请清空和清理接水盒.mp3', '接水盒已满，请清空和清理接水盒'], ['那我先去休息啦，有需要再喊我吧.mp3', '那我先去休息啦，有需要再喊我吧']]
    ...
    #              ]
    # text_list = [['饮品已制作完成，请及时清理渣盒哦.mp3', '饮品已制作完成，请及时清理渣盒哦']]
    text_list = [['咖啡豆已用完，正在取消饮品制作.mp3', '咖啡豆已用完，正在取消饮品制作']]
    ...
    for voice_id in voice_id_list:
        ...
        for text in text_list:
            submit_tts(create_time, voice_id, req_device_secret, text[0], text[1])
