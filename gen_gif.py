...
def create_gif(source,name,duration):
    ...
    '''   
    # 读取PNG文件并创建GIF
    with imageio.get_writer(name, mode='I', duration=duration, loop=0) as writer:  # duration为每帧间隔时间（秒）
...
def gen_gif(or_path, output, fps=10):
    ...
    duration_time = 1/fps # (1/fps) 每秒帧数
    # print(duration_time)
    # 生成gif
    create_gif(pic_list, gif_name, duration_time)

if __name__ == '__main__':    
    path='E:\\zzk_ota\\img\\png\\auto'
    gen_gif(path, "auto.gif", 12)

    path='E:\\zzk_ota\\img\\png\\bootanim'
    gen_gif(path, "bootanim.gif", 12)

    path='E:\\zzk_ota\\img\\png\\strong'
    gen_gif(path, "strong.gif", 12)

    path='E:\\zzk_ota\\img\\png\\wave'
    gen_gif(path, "wave.gif", 12)

    path='E:\\zzk_ota\\img\\png\\weak'
